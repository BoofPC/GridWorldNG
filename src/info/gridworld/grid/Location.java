...
/**
 * A <code>Location</code> object represents the row and column of a location in a two-dimensional
 * grid. <br />
...
public class Location implements Comparable {
  ...
  /**
   * Returns the direction from this location toward another location. The direction is rounded to
   * the nearest compass direction.
   * 
   ...
   */
  public int getDirectionToward(final Location target) {
    final int dx = target.getCol() - this.getCol();
    final int dy = target.getRow() - this.getRow();
    // y axis points opposite to mathematical orientation
    final int angle = (int) Math.toDegrees(Math.atan2(-dy, dx));
  ...
  /**
   * Compares this location to <code>other</code> for ordering. Returns a negative integer, zero, or
   * a positive integer as this location is less than, equal to, or greater than <code>other</code>.
   * Locations are ordered in row-major order. <br />
