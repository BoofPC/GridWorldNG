...
/**
 * <code>DisplayMap</code> is a collection that maps grid occupant classes to objects that know how
 * to display them. <br />
...
public class DisplayMap {
  ...
  private Display createDisplay(final Class<?> cl) {
    ...
    try {
      final ImageDisplay display = new ImageDisplay(cl);
      this.map.put(cl, display);
      return display;
    } catch (final Exception e) {
      // oh well...
  ...
  private class DisplayIcon implements Icon {
    ...
    public void paintIcon(final Component comp, final Graphics g, final int x,
      ...
      this.displayObj.draw(null, comp, g2,
        new Rectangle(x, y, this.getIconWidth(), this.getIconHeight()));
